...
NB_LATENTS = 2
FRAC       = 0.5
simples_y = np.random.binomial(1, 0.1, size=(NB_LATENTS, NB_CLASSES)) - 0.1
simples_x = np.random.binomial(1, 0.1, size=(NB_LATENTS, NB_CLASSES)) - 0.1

probs = np.zeros([NB_CLASSES,NB_CLASSES], dtype=np.float)
for sy, sx in zip(simples_y, simples_x):
  probs += np.outer(sy, sx)
probs += np.min(probs);  probs *= 1.0/(NB_CLASSES**2*np.mean(probs))
probs = (FRAC) * probs + (1.0-FRAC) * np.ones([NB_CLASSES,NB_CLASSES], dtype=np.float)/NB_CLASSES**2
print(probs)

W = 10 
img = np.zeros([NB_CLASSES*W,NB_CLASSES*W,3], dtype=np.float)
for y in range(NB_CLASSES):
  for x in range(NB_CLASSES):
    img[y*W:(y+1)*W , x*W:(x+1)*W] = np.array([1.0,1.0,1.0]) * probs[y,x]

plt.imsave('local-features-a.png', img)
